...
def parse_args():
    ...
    src.add_argument("--camera", type=int,
                     help="USB / UVC camera device index. For an Intel "
                          "RealSense plugged into USB, the RGB stream is "
                          "usually index 0 (sometimes 2 on Linux if the "
                          "depth stream registers first). Try `--camera 0` "
                          "and increment if you get a depth/IR image.")
    ...
    # ── Board geometry ──────────────────────────────────────────────────────
    # Lets you point the calibration loop at any ChArUco board, not just
    # the built-in 5×7 iPad pattern. All four dimensions are independent
    # so you can describe rectangular cells (e.g. the printed A3 board
    # with 31 mm wide × 33 mm tall cells — see --square_length_y).
    parser.add_argument(
        "--squares_x", type=int, default=5,
        help="Number of ChArUco squares across the board width "
             "(default: 5, the iPad pattern).",
    )
    parser.add_argument(
        "--squares_y", type=int, default=7,
        help="Number of ChArUco squares across the board height "
             "(default: 7, the iPad pattern).",
    )
    parser.add_argument(
        "--square_length_x", type=float, default=0.04,
        help="Physical width of a single chessboard square, in metres "
             "(default: 0.04 = 40 mm). For the printed A3 reference "
             "board with 31 mm × 33 mm cells, pass 0.031.",
    )
    parser.add_argument(
        "--square_length_y", type=float, default=0.0,
        help="Physical HEIGHT of a single chessboard square, in metres. "
             "Omit (or pass 0) for square cells — the code uses "
             "square_length_x in that case. Set this ONLY when your "
             "printed board has rectangular cells; e.g. pass 0.033 "
             "together with --square_length_x 0.031 for a 31×33 mm "
             "board. Internally we scale the 3-D object points in Y by "
             "square_length_y / square_length_x before calibrating, so "
             "the solver sees the true physical geometry.",
    )
    parser.add_argument(
        "--marker_length", type=float, default=0.02,
        help="Physical ArUco marker side, in metres. Should be smaller "
             "than square_length_x so there's a white border inside "
             "each black square (default: 0.02 = 20 mm, the iPad "
             "pattern). For a 31 mm cell, ~0.023 is typical.",
    )
    parser.add_argument(
        "--aruco_dict", type=str, default="DICT_4X4_50",
        help="ArUco dictionary the printed/displayed board was generated "
             "with. Must match exactly or detection will silently fail. "
             "Common choices: DICT_4X4_50 (iPad default), DICT_4X4_100, "
             "DICT_5X5_100, DICT_5X5_250. A 9×13 board needs at least "
             "≈59 markers, so a _50 dictionary is too small — use _100 "
             "or larger.",
    )
    parser.add_argument(
        "--legacy_pattern",
        ...
        default=False,
        help="Use OpenCV's pre-4.6 ChArUco marker-ID-to-position "
             "convention. Required for boards generated by calib.io, "
             "chev.me, or older Python scripts. Diagnostic: if "
             "detectMarkers finds every marker cleanly but the overlay "
             "shows 0 charuco corners / 0 samples, your PDF almost "
             "certainly uses the legacy layout — set this to true.",
    )
...
def main():
    ...
    # Translate the dictionary name into the OpenCV constant. We import
    # lazily so a typo'd --aruco_dict is caught with a clean error
    # instead of an AttributeError deep in cv2.
    from pattern_display import ARUCO_DICT_NAMES
    if args.aruco_dict not in ARUCO_DICT_NAMES:
        print(f"[ERROR] Unknown --aruco_dict: {args.aruco_dict}")
        print(f"        Choose from: {', '.join(sorted(ARUCO_DICT_NAMES))}")
        sys.exit(1)
    dict_id = ARUCO_DICT_NAMES[args.aruco_dict]

    # Non-square cell handling: build the board with an isotropic
    # square_length = square_length_x, then compute a y_scale factor
    # that the calibrator will use to stretch the 3-D object points in
    # Y. This keeps the upstream CharucoBoard API simple while still
    # producing correct intrinsics for rectangular-cell prints.
    if args.square_length_y and args.square_length_y > 0:
        y_scale = args.square_length_y / args.square_length_x
    else:
        y_scale = 1.0
    ...
    board, aruco_dict = create_board(
        ...
        square_length=args.square_length_x,
        marker_length=args.marker_length,
        aruco_dict_id=dict_id,
        legacy_pattern=args.legacy_pattern,
    )

    # Safety check: the dictionary must contain enough unique markers
    # for this board. A ChArUco board of N squares uses ceil(N/2)
    # markers (half the cells). Catching this here beats a silent
    # "only the first 50 cells decode" bug at runtime.
    n_markers_needed = (args.squares_x * args.squares_y + 1) // 2
    dict_size = int(args.aruco_dict.split("_")[-1])
    if n_markers_needed > dict_size:
        print(f"[ERROR] --aruco_dict {args.aruco_dict} has only {dict_size} "
              f"unique markers, but a {args.squares_x}×{args.squares_y} "
              f"board needs {n_markers_needed}. Use a larger dictionary "
              f"(e.g. DICT_4X4_100 or DICT_4X4_250).")
        sys.exit(1)

    cell_desc = (f"{args.square_length_x*1000:.1f}×"
                 f"{(args.square_length_y or args.square_length_x)*1000:.1f} mm")
    print(f"[INFO] ChArUco board: {args.squares_x}×{args.squares_y} squares, "
          f"{cell_desc} cells, marker {args.marker_length*1000:.1f} mm, "
          f"{args.aruco_dict}"
          + (f"  (y_scale={y_scale:.4f})" if y_scale != 1.0 else ""))
    ...
    while running:
        ...
        if detection.found and detection.num_corners >= 6:
            ...
            if do_capture and cap_ncorners >= 6:
                ...
                # Tier-1 acknowledgment: short non-speech ping fired the
                # instant a sample is captured. Bypasses the speech
                # cooldown entirely (uses a separate audio channel) so
                # the user *always* gets immediate audible confirmation
                # that the script saw their pose, even when the next
                # voice prompt is still on cooldown.
                guide.play_capture_ping()
                ...
                if should_try_trial:
                    ...
                    conf, trial_rms, trial_fx, err = run_trial_confidence(
                        ...
                        tracker, args.outlier_rms, y_scale=y_scale,
                    )
                    if conf is None:
                ...
                # Third exit condition: every reachable bin is full and we
                # have at least min_captures samples. Confidence may still
                # be below the threshold (e.g. because of structural
                # edge_gap floor with a printed board), but there is
                # nothing more the user can do — they cannot fill bins
                # that have already been skipped, and the loop would
                # otherwise spin forever asking for tilts that don't
                # exist. Skipping bins is already handled separately, so
                # progress[ax] == 1.0 for every axis means "all reachable
                # bins on every axis are full".
                #
                # tracker.progress() returns {axis: fill_fraction}, NOT a
                # scalar. Use min(values) so a single still-starved axis
                # blocks the exit.
                progress_per_axis = tracker.progress()
                min_progress = (min(progress_per_axis.values())
                                if progress_per_axis else 0.0)
                if (not finish_run
                        and n >= args.min_captures
                        and min_progress >= 1.0):
                    conf_str = (f"{live_confidence:.1f}"
                                if live_confidence is not None else "n/a")
                    print(f"  ✓ All reachable bins full at n={n} "
                          f"(confidence={conf_str}/100, "
                          f"target was {args.confidence_threshold:.0f}). "
                          f"Finishing — no more bins to capture.")
                    finish_run = True
        ...
        if (not args.no_voice
            ...
            if starved is not None:
                ax, bin_idx, deficit = starved
                # Tier-2: pass the current count and remaining need so
                # the guidance layer can speak progress-aware messages
                # ("good, one more like that") instead of repeating the
                # same instruction verbatim every cycle.
                cur_count = int(tracker.bins[ax][bin_idx])
                spoken = guide.suggest_bin(
                    ax, bin_idx, args.bins,
                    current=cur_count,
                    needed=int(deficit),
                )
